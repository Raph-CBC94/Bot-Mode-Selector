...
**How to apply:** Keep bounded AI waits and resilient send logic whenever changing the Discord message pipeline. Publish both the user's question and bot answer through the channel-message API in sequence; use a bounded background task only to meet Discord's interaction deadline and recreate the panel afterward.