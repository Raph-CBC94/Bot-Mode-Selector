...
6. Un administrateur peut utiliser `/panel` dans le salon autorisé pour créer ou mettre à jour le message panel. Le bouton **Poser une question** ouvre une modale avec un message obligatoire et un style facultatif (`insulte`, `suceur`, `vantard` ou `adulte`). Si le style est vide, le panel utilise le mode défini par `/mode`. Après chaque réponse du panel, l'ancien panel est supprimé et un nouveau est créé tout en bas du salon. Le bot doit pouvoir **Voir le salon**, **Lire l'historique des messages** et **Envoyer des messages**.
